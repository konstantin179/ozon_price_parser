...
def get_accounts_data(engine: Engine, client_id: str | int | list[int] = 'all'):
    ...
    attribute_name = {1: "client_id", 2: "api_key"}
    accounts_data = {}  # {acc_id: {"client_id": ,"api_key": }, }
    if client_id == 'all':
        query = """SELECT asd.account_id, asd.attribute_id, asd.attribute_value
                      FROM account_service_data asd
                     WHERE asd.attribute_id IN (1,2) AND
                         asd.account_id IN (
                            SELECT account_id
                              FROM (
                                SELECT *, row_number() OVER (
                                 ...
                                 WHERE asd.attribute_id = 1 and al.status_1 = 'Active'
                                   ) AS sq
                             WHERE row_number = 1)
